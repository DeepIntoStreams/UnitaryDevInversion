import os
import numpy as np
import pandas as pd
import torch
from sigkernel.static_kernels import LinearKernel, RBFKernel
from utils import FBM_data, AddTime, LaplaceKernel, Compare_test_metrics

if __name__ == '__main__':
    torch.manual_seed(0)
    np.random.seed(0)
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print(device)

    linear_kernel = LinearKernel()
    sigma = 0.1
    rbf_kernel = RBFKernel(sigma=sigma)
    laplace_kernel = LaplaceKernel(sigma=sigma)

    df_list = []
    for i in range(5):
        for h in [
                0.2, 0.25, 0.3, 0.35, 0.4, 0.425, 0.45, 0.475, 0.5, 0.525,
                0.55, 0.575, 0.6, 0.65, 0.7, 0.75, 0.8
        ]:
            print('trial =', i + 1)
            print('h =', h)

            test_X = AddTime(FBM_data(10000, dim=3, length=50, h=0.5)).double()
            test_Y = AddTime(FBM_data(10000, dim=3, length=50, h=h)).double()
            
            df = Compare_test_metrics(test_X, test_Y, device).run_HT(
                num_exp=20,
                num_perm=500,
                sample_size=200,
                tag=h,
                static_kernel=linear_kernel,
                # static_kernel=rbf_kernel,  # uncomment this line to use the RBF kernel
                # static_kernel=laplace_kernel,  # uncomment this line to use the Laplace kernel
                dyadic_order=0,
            )
            print(df)
            df_list.append(df)
            df = pd.concat(df_list)

            if not os.path.exists('numerical_results'):
                os.mkdir('numerical_results')
                df.to_csv(f'numerical_results/sig mmd trial={i + 1} h={h}.csv')