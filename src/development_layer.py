...
class development_layer(nn.Module):
    def __init__(
        ...
        Args:
            ...
            lie_group (str) : Choice of the distance.
        ...
        if lie_group == "PCFD":
            self.projection = unitary_projection(input_size,
                                                 hidden_size,
                                                 channels,
                                                 init_range=init_range)
            self.complex = True
        elif lie_group == "OPCFD":
            self.projection = orthogonal_projection(input_size, hidden_size,
                                                    channels, init_range)
            self.complex = False
        elif lie_group == "RPCFD":
            self.projection = orthogonal_diag_projection(
                input_size, hidden_size, channels, init_range)
            self.complex = False
        else:
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.include_initial:
            input = torch.cat([torch.zeros((N, 1, C)).to(input.device), input],
                              dim=1)
