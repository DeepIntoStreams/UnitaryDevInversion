...
def orthogonal_lie_init_(tensor: torch.tensor, init_=None):
    ...
    real_part = (upper_tri_real - upper_tri_real.transpose(
        -2, -1)) / torch.tensor([2], device=tensor.device).sqrt()
...
class orthogonal(nn.Module):
    """
    A PyTorch module for parametrizing a special orthogonal Lie algebra using a general linear matrix.
    """
    def __init__(self, size):
        """
        Initializes the special orthogonal module.
    ...
    def frame(X: torch.tensor) -> torch.tensor:
        """
        Parametrizes the special orthogonal Lie algebra from the general linear matrix X.
    ...
    def forward(self, X: torch.tensor) -> torch.tensor:
        ...
        Returns:
            torch.Tensor: A tensor parametrized in the special orthogonal Lie algebra.
    ...
    def in_lie_algebra(X, eps=1e-5):
        ...
        """
        return (X.dim() >= 2 and X.size(-2) == X.size(-1)
